...
export default function MaintenanceAddModal({ onClose, onSave, vehicles = [] }) {
  ...
  const addService = () => {
    setForm((p) => ({ ...p, services: [...p.services, { id: Date.now(), name: "", price: "" }] }));
  };

  const updateService = (id, field, value) => {
    setForm((p) => ({ ...p, services: p.services.map((s) => (s.id === id ? { ...s, [field]: value } : s)) }));
  };
        ...
        <form className="ma-add-form" onSubmit={handleSubmit}>
          <div className="ma-add-body">
            ...
            <div>
              ...
              <input className="input" value={form.invoiceNumber} onChange={(e) => update("invoiceNumber", e.target.value)} />
            </div>
            <div>
          ...
          <div className="grid two mt-3">
            ...
            <div>
              ...
              <select className="input" value={form.maintenanceType} onChange={(e) => update("maintenanceType", e.target.value)}>
          ...
          <div className="grid two mt-3">
            <div>
              ...
              <input className="input" value={form.technician} onChange={(e) => update("technician", e.target.value)} placeholder="Tên kỹ thuật viên" />
            </div>
            <div>
              <label>Xưởng sửa chữa *</label>
              <input className="input" value={form.workshop} onChange={(e) => update("workshop", e.target.value)} placeholder="Tên xưởng" />
            </div>
          </div>
          ...
          <div className="services-list">
            {form.services.length === 0 ? (
              <div className="empty-services">Chưa có dịch vụ nào. Nhấn "Thêm dịch vụ" để bắt đầu.</div>
            ) : null}
              ...
              <div className="service-card" key={s.id}>
                <div className="service-left">
                  ...
                  >
                    ...
                    <option value="Thay dầu động cơ - 500000">Thay dầu động cơ - 500,000đ</option>
                    <option value="Kiểm tra phanh - 300000">Kiểm tra phanh - 300,000đ</option>
                    <option value="Thay nhớt hộp số - 700000">Thay nhớt hộp số - 700,000đ</option>
                  </select>
                </div>
                ...
                <div className="service-mid">
                  ...
                  <input
                    ...
                    min="1"
                    value={s.qty || 1}
                    onChange={(e) => updateService(s.id, "qty", Math.max(1, Number(e.target.value || 1)))}
                  />
                </div>
                ...
                <div className="service-right">
                  ...
                  />
                  <button type="button" className="btn-trash" onClick={() => removeService(s.id)} title="Xóa dịch vụ">
                    <FaTrash />
                  </button>
                </div>
              </div>
            ...
            <div className="total-row">
              ...
              <div className="total-amount">{formatCurrency(computeTotal(form.services))}</div>
            </div>
            </div>
          </div>
          ...
          <div className="grid two">
            <div>
              ...
              <input className="input" type="date" value={form.nextDate} onChange={(e) => update("nextDate", e.target.value)} />
            </div>
            <div>
              <label>Số km bảo trì tiếp theo</label>
              <input className="input" placeholder="VD: 15000" value={form.nextKm} onChange={(e) => update("nextKm", e.target.value)} />
            </div>
          </div>

          <div className="mt-3">
            <label>Ghi chú</label>
            <textarea className="input" rows={4} value={form.notes} onChange={(e) => update("notes", e.target.value)} placeholder="Ghi chú về bảo trì" />
          </div>
